...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if mri_modelpath is not None:
            ...
            self.load_state_dict(mri_emb_weight, strict=False)
        ...
        self._make_clf_layer(block_inplanes, block, num_classes)
    ...
    def _make_clf_layer(self, block_inplanes, block, num_classes):
        if self.decouple:
    ...
    def forward(self, x: torch.Tensor, demo_feature=None, get_inter=False, get_before_avg=False) -> torch.Tensor:
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        if self.decouple:
            ...
            tmp = 0
            for i in range(2):
                tmp += self.fc[i](x).squeeze() * (i == age_mask)
            y_hat = tmp.squeeze()
        else:
