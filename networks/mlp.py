import torch
import torch.nn as nn
from torch.autograd import Function
from torch.nn import functional as F

class MLP(nn.Module):
    def __init__(self, feature_size, hidden_dim, num_classes, num_layer=3, with_dropout=False,
                 adv=False, adv_lambda=1., relu_output=False, act='relu'):
        super(MLP, self).__init__()
        try:  # list
            in_features = self.compute_input_size(feature_size)
        except:  # int
            in_features = feature_size

        self.adv = adv
        self.relu_output = relu_output
        self.act = act
        if adv:
            self.adv_lambda = adv_lambda
        self._make_layer(in_features, hidden_dim, num_classes, num_layer, with_dropout)


    def forward(self, feature, demo_feature=None, get_inter=False):
        # feature = torch.flatten(feature, 1)

        if self.adv:
            feature = ReverseLayerF.apply(feature, self.adv_lambda)

        h = self.features(feature)
        out = self.head(h)
        if self.relu_output:
            out = F.relu(out)
        out = out.squeeze()
        if get_inter:
            return h, out
        else:
            return out

    def compute_input_size(self, feature_size):
        in_features = 1
        for size in feature_size:
            in_features = in_features * size

        return in_features

    def _make_layer(self, in_dim, h_dim, num_classes, num_layer, with_dropout=False):

        num_outputs = 1 if num_classes <= 2 else num_classes
        if num_layer == 1:
            self.features = nn.Identity()
            h_dim = in_dim
        else:
            features = []
            for i in range(num_layer - 1):
                features.append(nn.Linear(in_dim, h_dim) if i == 0 else nn.Linear(h_dim, h_dim))
                if self.act == 'relu':
                    features.append(nn.ReLU())
                elif self.act == 'silu':
                    features.append(nn.SiLU())
                if with_dropout:
                    features.append(nn.Dropout(0.5))
            self.features = nn.Sequential(*features)

        self.head = nn.Linear(h_dim, num_outputs)


class ReverseLayerF(Function):

    @staticmethod
    def forward(ctx, x, alpha):
        ctx.alpha = alpha

        return x.view_as(x)

    @staticmethod
    def backward(ctx, grad_output):
        return grad_output.neg() * ctx.alpha, None