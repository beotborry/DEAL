...
from networks.film_utils import *
...
class ResNetBlock(nn.Module):
    ...
    def __init__(
        ...
        film_input_dim = 3,
        film_only_last=False,
        input_demo_for_film='emd',
        age_mean=None,
        age_std=None,
        decouple_standardize=False,
    ) -> None:
        """
        Args:
        ...
        self.bn2 = norm_type(planes, affine=film_only_last)
        self.downsample = downsample
        self.stride = stride
        self.film_only_last = film_only_last
        self.input_demo_for_film = input_demo_for_film
        self.age_mean = age_mean
        self.age_std = age_std
        self.decouple_standardize = decouple_standardize
        if not film_only_last:
            self.film_generator = nn.Linear(film_input_dim, 2 * planes)
            self.film = FiLM()
    ...
    def forward(self, x: torch.Tensor, demo_feature) -> torch.Tensor:
        ...
        out = self.conv2(out)
        out = self.bn2(out)
        if not self.film_only_last:
            film_input = get_film_input(demo_feature, self.input_demo_for_film, age_mean=self.age_mean,
                                        age_std=self.age_std, decouple_standardize=self.decouple_standardize)
            film_vector = self.film_generator(film_input).view(out.size(0), self.planes, 2)
            beta = film_vector[:, :, 0]
            gamma = film_vector[:, :, 1]
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if 'daft' in input_demo_for_film:
            ...
            if 'faq' in input_demo_for_film:
                input_demo_for_film.extend(['has_faq'])
            if 'apoe4' in input_demo_for_film:
                input_demo_for_film.extend(['has_apoe4'])

            film_input_dim = len(input_demo_for_film)

        self.layer1 = self._make_layer(block, block_inplanes[0], layers[0], spatial_dims, shortcut_type,
                                       film_input_dim=film_input_dim, film_only_last=film_only_last,
                                       input_demo_for_film=input_demo_for_film, age_mean=age_mean, age_std=age_std,
                                       )
        ...
        if decouple:
            ...
            for _ in range(2):
                self.fc.append(nn.Linear(block_inplanes[3] * block.expansion, num_classes))
        else:
        ...
        if film_aux_net_act == 'relu':
            film_aux_net_act = nn.ReLU()
        elif film_aux_net_act == 'gelu':
            film_aux_net_act = nn.GELU()
        elif film_aux_net_act == 'silu':
            film_aux_net_act = nn.SiLU()

        if mri_modelpath is not None:
            self.load_state_dict(torch.load(mri_modelpath), strict=film_only_last)
            print("load pretrained model succesfully")


        self.decouple = decouple

        clf_input_dim = block_inplanes[3] * block.expansion

        if decouple:
            self.fc = nn.ModuleList()
            for _ in range(2):
                self.fc.append(nn.Linear(clf_input_dim, num_classes))
        elif not decouple and mri_modelpath is not None:
            self.fc = nn.Linear(clf_input_dim, num_classes,) if feed_forward else None
        else:
            pass
        ...
        self.age_mean = age_mean
        self.age_std = age_std 
        self.film_aux_net = film_aux_net


        if self.film_only_last:
            if film_aux_net == 'linear':
                self.film_generator = nn.Linear(film_input_dim, 2 * block_inplanes[3] * block.expansion)
            else:
                film_generator = nn.Sequential(
                    nn.Linear(film_input_dim, 512),
                    film_aux_net_act,
                    nn.Linear(512, 2* block_inplanes[3] * block.expansion)
                )
                self.film_generator = film_generator

        self.film = FiLM(film_for_last=True)
    ...
    def _make_layer(
        ...
        shortcut_type: str,
        stride: int = 1,
        film_input_dim: int = 3,
        film_only_last: bool = False,
        input_demo_for_film: str = 'aem',
        age_mean=None,
        age_std=None,
    ) -> Sequential:
        conv_type: Callable = Conv[Conv.CONV, spatial_dims]
    ...
    def forward(self, x: torch.Tensor, demo_feature=None, get_inter=False) -> torch.Tensor:
        ...
        x = self.layer1(x, demo_feature)
        x = self.layer2(x, demo_feature)
        x = self.layer3(x, demo_feature)
        ...
        if self.film_only_last:
            ...
            film_vector = self.film_generator(film_input).view(x.size(0), x.size(1), 2)
            beta = film_vector[:, :, 0]
            gamma = film_vector[:, :, 1]
        ...
        if self.decouple:
            tmp = 0
            group = demo_feature[14]
            mask = (group % 2 == 1).cuda()
            for i in range(2):
                tmp += self.fc[i](x).squeeze() * (i == mask)
            y_hat = tmp.squeeze()
        else:
