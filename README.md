...
- **Fairness**: Group DRO, SUBG, and age-based classifier decoupling
...
| **Fairness** | `--group-dro`, `--mri-subg`, `--decouple`, `--group-weight-lr` |
...
- `scripts/run_DEAL.sh` – example runs for `ad_time_cox` with multiple learning rates
...
W&B project name is `DEAL`; disable with `WANDB_MODE=disabled` if needed.
